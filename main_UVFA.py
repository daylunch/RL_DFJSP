...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 23:15:40 2024

@author: Administrator

Sequence dependent setup time: UVFA
"""
...
class DQNAgent:
    def __init__(self, state_size, action_size, hidden_size=128, learning_rate=3e-5, gamma=0.99,
                 epsilon=1.0, epsilon_decay=0.999997, epsilon_min=0.01, memory_size=20000, batch_size=256):
...
def handle_operation_completion(machine, work_centers, processing_times, rl_agents, simulation_clock, event_queue, completed_jobs, work_centers_machines):
    ...
    if next_wc_id is not None:
        ...
        # Make scheduling decision upon job arrival in the next work center
        decision_making(
            work_centers[next_wc_id],
            rl_agents[next_wc_id],
            simulation_clock,
...
def calculate_reward(work_center, simulation_clock, processing_times, original_waiting_jobs, sorted_waiting_jobs):
    ...
    def simulate_sequence(jobs_sequence, use_dynamic_priority, copied_work_center):
        ...
        while jobs_in_sequence:
            ...
            elif use_dynamic_priority == 'tardiness':
                jobs_in_sequence = apply_job_priority(
                    jobs_in_sequence,
                    'Shortest_Setup_Adjusted_Processing_Time',
                    processing_times,
                    copied_work_center.work_center_id,
                    copied_work_center
                )
            elif use_dynamic_priority == 'setup':
                jobs_in_sequence = apply_job_priority(
                    jobs_in_sequence,
                    'Shortest_Setup_Time',
                    processing_times,
    ...
    original_completion_times, _ = simulate_sequence(
        original_waiting_jobs, use_dynamic_priority='tardiness', copied_work_center=copied_work_center_original
    )
    copied_work_center_original = copy.deepcopy(work_center)
    _, original_setup_time = simulate_sequence(
        original_waiting_jobs, use_dynamic_priority='setup', copied_work_center=copied_work_center_original
    )
    copied_work_center_sorted = copy.deepcopy(work_center)
    sorted_completion_times, sorted_setup_time = simulate_sequence(
        sorted_waiting_jobs, use_dynamic_priority='RL', copied_work_center=copied_work_center_sorted
    )
    ...
    time_to_due_diff = (avg_time_to_due_after - avg_time_to_due_before)/100
    setup_time_diff = (original_setup_time - sorted_setup_time)/100
    
    # Combine the rewards
    reward = weights['time_to_due'] * time_to_due_diff + weights['setup_time'] * setup_time_diff  # Adjust weights if needed
    return reward
...
def plot_metrics(metrics):
    # Ensure that matplotlib does not block the execution
    import matplotlib
    matplotlib.use('Agg')

    # Plot Makespan over Episodes
    ...
    # Plot Average Flow Time over Episodes
    plt.figure(figsize=(10, 6))
    ...
    # Plot Average Tardiness over Episodes
    plt.figure(figsize=(10, 6))
    ...
    # Plot Machine Utilization over Episodes
    plt.figure(figsize=(10, 6))
...
def plot_cumulative_rewards(cumulative_rewards_per_agent):
    for wc_id, rewards in cumulative_rewards_per_agent.items():
        ...
        plt.close()
        print(f"Cumulative reward plot saved for Work Center {wc_id} as 'cumulative_reward_wc{wc_id}.png'")
...
def train_rl_agent(num_episodes, total_num_jobs, rl_agents, update_target_every, mean_interval_range, num_machines_per_wc_range):
    ...
    global weights  # So we can overwrite it each episode
    weights = {
        'time_to_due': 0.5,
        'setup_time': 0.5
    }
    loss_tracking = {wc_id: [] for wc_id in rl_agents.keys()}
    cumulative_rewards_per_agent = {wc_id: [] for wc_id in rl_agents.keys()}

    # Set the number of work centers to a fixed value
    num_work_centers = 5  # Set to the desired fixed number of work centers

    for episode in tqdm.tqdm(range(1, num_episodes + 1)):
        w1 = random.random()
        w2 = 1.0 - w1
        
        # 3b) Overwrite the global weights
        weights['time_to_due'] = w1
        weights['setup_time'] = w2
        # Reset cumulative loss and counts
        cumulative_loss = {wc_id: 0.0 for wc_id in rl_agents.keys()}
        count = {wc_id: 0 for wc_id in rl_agents.keys()}
        ...
        # Number of job types can still be random or fixed as desired
        min_num_job_types = 1  # Adjust as needed
        max_num_job_types = 15
        num_job_types = random.randint(min_num_job_types, max_num_job_types)

        # Generate job_types and job_type_routing according to your requirements
        job_types = {}
        job_type_routing = {}
        for jt in range(1, num_job_types + 1):
            # Generate a random permutation of all work centers
            num_operations = random.randint(2, num_work_centers)
            routing = random.sample(range(num_work_centers),num_operations)
            # For each work center in routing, assign a processing time in [1,99]
            operations = {}
            for wc_id in routing:
                processing_time = random.randint(1, 99)
                operations[wc_id] = {'processing_time': processing_time}
            # Store in job_types and job_type_routing
            job_type = f'Type{jt}'
            job_types[job_type] = {'operations': operations}
            job_type_routing[jt] = routing  # Store the ordered routing

        # Generate num_machines_per_wc and work_centers_machines
        num_machines_per_wc = {}
        work_centers_machines = {}
        for wc_id in range(num_work_centers):
            ...
            logging.debug(f"Work Center {wc_id} Machines: {work_centers_machines[wc_id]}")
        
        
        # Define processing_times based on job_types
        processing_times = {jt: job_types[jt]['operations'] for jt in job_types}

        # Randomly select mean interval for each job type from integer interval
        job_type_mean_intervals = {
            jt: random.randint(mean_interval_range[0], mean_interval_range[1])
            for jt in range(1, num_job_types + 1)
        }

        # Set up job type distributions based on the random mean intervals
        ...
        # Log the number of work centers and job types
        logging.info(f"Number of Work Centers: {num_work_centers}")
        logging.info(f"Number of Job Types: {num_job_types}")

        logging.info(f"Job Type Mean Intervals: {job_type_mean_intervals}")
        ...
        logging.info("Work Center Machine Numbers:")
        print("Work Center Machine Numbers:")
        for wc_id in sorted(num_machines_per_wc.keys()):
            num_machines = num_machines_per_wc[wc_id]
            logging.info(f"  Work Center {wc_id}: {num_machines} machines")
            print(f"  Work Center {wc_id}: {num_machines} machines")
        ...
        if episode % update_target_every == 0:
            for wc_id, agent in rl_agents.items():
                ...
                logging.info(f"Updated target network for Work Centre ID {wc_id} at Episode {episode}")
                print(f"Updated target network for Work Centre ID {wc_id} at Episode {episode}")
        ...
        # Aggregate Loss Data
        for wc_id, agent in rl_agents.items():
            # Retrieve the latest average loss from metrics
            avg_loss = metrics['Average_Loss'][-1][wc_id]
            cumulative_loss[wc_id] += avg_loss
            cumulative_rewards_per_agent[wc_id].append(agent.cumulative_reward)
            count[wc_id] += 1

        # Calculate average loss per agent for this episode
        for wc_id in rl_agents.keys():
            if count[wc_id] > 0:
                average_loss = cumulative_loss[wc_id] / count[wc_id]
            else:
                average_loss = 0.0
            loss_tracking[wc_id].append(average_loss)

        # Plot Loss Every `update_target_every` Episodes
        if episode % update_target_every == 0:
            for wc_id, losses in loss_tracking.items():
                plt.figure(figsize=(10, 6))
                plt.plot(range(1, len(losses) + 1), losses, label=f'Work Centre {wc_id}')
                plt.xlabel('Episode')
                plt.ylabel('Average Loss')
                plt.title(f'Average Loss per Episode for Work Centre {wc_id}')
                plt.legend()
                plt.tight_layout()
                plot_filename = f'average_loss_wc{wc_id}_up_to_episode_{episode}.png'
                plt.savefig(plot_filename)
                plt.close()
                logging.info(f"Average loss plot for Work Center {wc_id} saved as '{plot_filename}'.")
                print(f"Average loss plot for Work Center {wc_id} saved as '{plot_filename}'.")
...
if __name__ == "__main__":
    ...
    total_num_jobs = 100  # Total number of jobs to release
    update_target_every = 100  # Update target network every N episodes

    # Mean interval range (integer values)
    mean_interval_range = [5, 15]  # Mean intervals are integers from 5 to 15 inclusive

    # Number of machines per work center range
    num_machines_per_wc_range = [1, 5]  # Number of machines per work center, integers between 1 and 5 inclusive

    # Set the number of work centers to a fixed value
    num_work_centers = 5  # Set to the desired fixed number of work centers

    # Initialize RL Agents for each work center
    max_num_work_centers = num_work_centers  # Set maximum number of work centers based on your fixed value
    ...
    action_size = len(job_priority_rules) * len(machine_priority_rules)  # 10 * 1 = 10

    rl_agents = {}
    for wc_id in range(max_num_work_centers):
        state_size = 43  # Adjust based on your state representation
        rl_agents[wc_id] = DQNAgent(state_size, action_size)
    ...
    # Train the RL agents
    cumulative_rewards = train_rl_agent(
        num_episodes,
        total_num_jobs,
        rl_agents,
        update_target_every,
    ...
    # Save the trained models for each agent
    for wc_id, agent in rl_agents.items():
        model_save_path = f'dqn_policy_net_wc{wc_id}.pth'
        torch.save(agent.policy_net.state_dict(), model_save_path)
        logging.info(f"Model for Work Center ID {wc_id} saved to {model_save_path}.")
        print(f"Model for Work Center ID {wc_id} saved to {model_save_path}.")
